...
async function sendRegistrationOtp(email, otp) {
  const resendApiKey = process.env.RESEND_API_KEY ? process.env.RESEND_API_KEY.trim().replace(/^['"]|['"]$/g, '') : undefined;
  ...
  if (resendApiKey) {
    try {
      console.log(`✉️  [Resend Mailer] Dispatching OTP email via Resend HTTPS API...`);
      const htmlContent = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff;">
          <div style="text-align: center; margin-bottom: 24px; padding-bottom: 16px; border-bottom: 2px solid #3b82f6;">
      ...
      const payload = JSON.stringify({
        from: 'MediCore Healthcare <onboarding@resend.dev>',
        to: email,
      ...
      console.log(`✉️  [Resend Mailer] Real email dispatched successfully via Resend HTTPS. MessageId: ${result.id}`);
      return { sent: true, method: 'smtp', info: result };
    } catch (error) {
      console.error(`❌ [Resend Mailer] Failed to send real email via Resend HTTPS API:`, error.message);
      console.log(`ℹ️  Falling back to standard SMTP / console...`);
    }
  }
...
function checkSmtpConfigured() {
  const resendApiKey = process.env.RESEND_API_KEY ? process.env.RESEND_API_KEY.trim() : undefined;
